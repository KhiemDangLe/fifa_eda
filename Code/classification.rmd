...
```{r}
glimpse(preferred_foot_under)

preferred_foot_under <- preferred_foot_under |> 
  dplyr::select(-c(id, name, nationality, jersey_number, loaned_from, club))



out_md <- glm(formula = preferred_foot ~ overall + position + crossing + curve + weak_foot, , data = preferred_foot_under, family = binomial)

out_md <- glm(formula = preferred_foot ~ weak_foot + overall + ball_control+finishing+crossing+agility+strength+positioning+short_passing+long_passing, data =preferred_foot_under,
family = binomial)
```
...
```{r}
out_nb <- naiveBayes(formula = preferred_foot ~ overall + position + crossing + curve + weak_foot,
